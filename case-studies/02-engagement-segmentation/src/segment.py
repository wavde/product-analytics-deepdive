...
Pulls four engagement features per active user from DuckDB:
    sessions_28d, distinct_active_days_28d, purchase_count, total_revenue_cents
...
features AS (
    SELECT
        e.user_id,
        COUNT(*) FILTER (WHERE e.event_name = 'view_product')              AS sessions_28d,
...
SELECT * FROM features WHERE sessions_28d > 0
"""
...
def fit_kmeans(
    ...
    cols = ["sessions_28d", "active_days_28d", "purchase_count", "revenue_cents"]
...
def cluster_summary(labelled: pd.DataFrame) -> pd.DataFrame:
    ...
    return summary[["cluster", "n_users", "sessions_28d", "active_days_28d",
...
def label_clusters(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in out.iterrows():
        if row["revenue_dollars"] > out["revenue_dollars"].quantile(0.75):
